...
const Home = () => {
  ...
  const fetchRestaurantApi = async () => {
    ...
    setActivityCategoryId(updatedData[0].menuCategoryId)
    setIsLoading(false)
  }
  ...
  const onUpdatedActiveCategoryIdx = menuCategoryId =>
    setActivityCategoryId(menuCategoryId)
      ...
      return (
        <li
          className={`each-tab-item ${
            eachCategory.menuCategoryId === activityCategoryId
              ? 'active-tab-item'
              : ''
          }`}
  ...
  const renderDishes = () => {
    const {categoryDishes} = response.find(
      eachCategory => eachCategory.menuCategoryId === activityCategoryId,
    )

    return (
      <ul className="m-0 dlflex flex-column dishes-list-container">
